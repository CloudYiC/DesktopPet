...
export function PacketTemplateDialog({ open, count, search, busy, feedback, returnFocus, searchRef, children, onSearch, onImport, onExport, onClose }: PacketTemplateDialogProps) {
  ...
  useEffect(() => {
    ...
    if (open && !element.open) {
      ...
      if (target?.isConnected) target.focus({ preventScroll: !returnFocus.current });
    }
  ...
  return createPortal(<dialog ref={dialog} className={styles.dialog} data-testid="packet-library-panel" tabIndex={-1} aria-labelledby={titleId} aria-modal="true" onCancel={(event) => { event.preventDefault(); onClose(); }} onKeyDown={(event) => {
    ...
    const element = dialog.current;
    if (!element) return;
    const focusable = Array.from(element.querySelectorAll<HTMLElement>('button:not(:disabled), input:not(:disabled), [tabindex]:not([tabindex="-1"])')).filter((item) => item.getClientRects().length > 0);
    ...
    <div className={styles.toolbar}>
      ...
      <button disabled={busy} onClick={onImport}>导入</button><button onClick={onExport}>导出</button>
    </div>
    {feedback && <p className={feedback.error ? styles.error : styles.notice} role={feedback.error ? 'alert' : 'status'}>{feedback.text}</p>}
    <div className={styles.list} data-testid="packet-library">{children}</div>
    <footer className={styles.footer}><span>载入只填入编辑区，不会连接或发送。</span><button onClick={onClose}>完成</button></footer>
