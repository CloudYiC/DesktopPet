...
- 网络、串口和 Modbus 调试助手在视口宽度超过 900px 时统一左右分栏，只有较窄窗口才上下排列；左侧地址与端口配对，发送选项紧凑排列，右侧日志或结果保持可见。矮窗口、放大字号或展开额外参数时，左侧允许必要的内部滚动，不通过缩小字体或裁掉按钮隐藏内容；风险确认和错误反馈保留
...
```text
out/dist/CloudYiAssistant-Setup-0.13.9.exe
```
...
把 `CloudYiAssistant-Setup-0.13.9.exe` 复制到 Windows 10/11 x64 电脑并双击，
