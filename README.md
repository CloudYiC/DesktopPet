...
- 首次启动时展示云依助手主界面
- 云依工具分类位于侧边栏顶部，宠物功能随后排列，助手设置固定在底部；代码与数据库工具统一归入“数据处理”，图片转换统一归入“文件处理”，不再显示独立代码、AI、图片或数据库分类
...
- 软件卸载采用左软件列表内部滚动、右详情固定分页的布局，分类、路径详情与清理按钮在默认窗口中保持可见；完整名称与风险确认只在点击清理后弹出
- 开始菜单嵌套目录和当前/公共桌面的 `.lnk` 会按实际本地目标核对关联，不要求父目录名称匹配发布者；后台扫描可取消，卸载后可仅复查已保存候选，不删除共享父目录
- 最外侧导航统一使用线条 SVG 图标：主页、数据库、地球、扳手、文件夹、日历、清单、状态与齿轮；不增加字体包或改变侧栏和窗口尺寸
...
- 工具分类页沿用 CloudYiCSC 的紧凑目录并保留名称搜索；所有内置工具始终可用，旧版保存在 WebView2 中的停用状态会被忽略
- 结束进程、网络监听、软件卸载与残留清理、数据库写入等敏感操作继续由各自的原生校验和确认流程保护，不依赖界面显示状态
...
```text
out/dist/CloudYiAssistant-Setup-0.13.1.exe
```
...
把 `CloudYiAssistant-Setup-0.13.1.exe` 复制到 Windows 10/11 x64 电脑并双击，
