...
- 打开依依工作台时暂时隐藏桌面角色；最小化不会恢复，关闭工作台后才重新出现
- 工作台打开期间的到时事项仍会提示音、托盘通知并更新列表，关闭工作台后再由角色演出提醒
- 依依工作台按需加载，减少平时常驻资源占用
- 首次启动时展示依依工作台
- CloudYiCSC 工具分类位于侧边栏顶部，宠物功能随后排列，模块管理与工作台中心固定在底部；代码与数据库工具统一归入“数据处理”，图片转换统一归入“文件处理”，不再显示独立代码、AI、图片或数据库分类
...
- 模块管理只展示已经可用的内置工具；工作台中心包含三套浅色主题、界面字号、上次页面恢复、模块状态、本机数据概览和版本信息
...
```text
out/dist/YiyiWorkbench-Setup-0.11.6.exe
```
...
把 `YiyiWorkbench-Setup-0.11.6.exe` 复制到 Windows 10/11 x64 电脑并双击，
