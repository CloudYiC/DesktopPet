...
可爱依依是一位会动的桌面小助手，也是一套本地优先的 CloudYi 桌面工作台。项目使用 C 与 C++11 提供 Windows 原生能力，使用 React 18 构建角色、事项与工具界面。
...
- 单实例保护，重复启动时直接打开已有桌面工作台
- 桌面工作台按需加载，减少平时常驻资源占用
- 首次启动时展示桌面工作台
...
```text
out/dist/CuteYiyiDesktopPet-Setup-0.11.1.exe
```
...
把 `CuteYiyiDesktopPet-Setup-0.11.1.exe` 复制到 Windows 10/11 x64 电脑并双击，
