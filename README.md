...
- 左侧连接与内容设置内部滚动，发送操作固定；右侧日志保留至少 320px 正文阅读高度，空报文库紧凑显示并可收起。矮窗口、大字号或状态换行时允许详情外侧自然纵向滚动，不压扁日志，也不随记录增加撑高。停止发送保留连接，断开连接才释放端口；阅读旧日志时新消息不抢走滚动位置
...
```text
out/dist/CloudYiAssistant-Setup-0.13.10.exe
```
...
把 `CloudYiAssistant-Setup-0.13.10.exe` 复制到 Windows 10/11 x64 电脑并双击，
