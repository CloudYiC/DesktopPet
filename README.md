...
- 独立桌面“发包工具”：TCP/UDP、文本/HEX/转义字节、二进制文件载入、常用报文库导入导出、有限次数重发和响应记录；可选择本机网卡、指定组播发送接口和 TTL、单独加入/退出组播接收，显示真实监听端口与目标；外部网络操作经居中确认，左侧发送按钮固定，不在独立网页端提供通信
- 发包日志保留至少 320px 正文阅读高度，空报文库紧凑显示并可手动收起；窗口较矮、字号较大或状态换行时允许详情外侧自然纵向滚动，不压扁日志，也不随记录增加撑高。网络调试助手继续用于连接、TCP 服务端与持续收发，发包工具专注报文保存、载入和批次发送
...
- 21 个桌面工具全部随安装包内置并直接可用；工具卡片只提供中文“打开”操作，不再显示启用状态、技术标签或状态筛选
...
```text
out/dist/CloudYiAssistant-Setup-0.13.8.exe
```
...
把 `CloudYiAssistant-Setup-0.13.8.exe` 复制到 Windows 10/11 x64 电脑并双击，
