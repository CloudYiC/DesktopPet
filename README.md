...
- 0.13.16 移除工具首页顶部介绍横幅和侧栏名称下的小字；首页从分类卡片直接开始，保留搜索、工具入口与原有间距
...
- 自动收起按整台电脑的键鼠空闲时间计时，恢复操作后展开；举牌结束后的未完成事项不再永久阻止收起，收起时保留可见的部分角色而非仅露出透明窗口边缘。显隐规则与验证边界见 [桌宠窗口行为](docs/pet-window-behavior.md)
...
```text
out/dist/CloudYiAssistant-Setup-0.13.16.exe
```
...
把单个 `CloudYiAssistant-Setup-0.13.16.exe` 通过 U 盘复制到 Windows 10/11 x64 电脑并双击，
