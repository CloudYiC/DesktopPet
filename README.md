...
依依工作台是一套带有会动角色“粉色依依”的本地优先 Windows 工具工作台。项目使用 C 与 C++11 提供 Windows 原生能力，使用 React 18 构建角色、事项与工具界面。
...
- SQLite 本地存储与每秒提醒调度
- WebView2 JSON 双向通信
- Windows 原生提醒提示与系统托盘菜单，窗口、托盘、程序和安装包统一使用依依图标
...
- 单实例保护，重复启动时直接打开已有依依工作台
- 依依工作台按需加载，减少平时常驻资源占用
- 首次启动时展示依依工作台
- CloudYiCSC 工具分类位于侧边栏顶部，宠物功能随后排列，插件商店与 CloudYi 中心固定在底部；代码与数据库工具统一归入“数据处理”，图片转换统一归入“文件处理”，不再显示独立代码、AI、图片或数据库分类
...
- 插件商店会明确区分“已内置”和“待迁移”工具；CloudYi 中心包含账户入口、三套浅色主题、界面字号、上次页面恢复、插件状态、本机数据概览和版本信息，不会伪造云端登录
...
- 插件管理支持搜索、权限说明以及内置工具的本地登记和移除，状态保存在本地 WebView2 配置中
- 工具分类页沿用 CloudYiCSC 的紧凑目录：搜索、状态筛选、Local/Available/Coming Soon 标记以及 Open/Install 操作
...
```text
out/dist/YiyiWorkbench-Setup-0.11.5.exe
```
...
把 `YiyiWorkbench-Setup-0.11.5.exe` 复制到 Windows 10/11 x64 电脑并双击，
